...
export function CalculatorPage() {
        ...
        mutationFn: async () => {
            ...
            const details = data.map(row => {
                ...
                return {
                    ...
                    additional_sales: row.additionalSales || 0
                };
            });
    ...
    const handleDataLoaded = (loadedData: IncentiveData[]) => {
        const mergedData = loadedData.map(row => {
            const employee = employees?.find(e => e.name === row.name);
            if (employee) {
                return {
                    ...row,
                    position: employee.position, // Override position
                    category: employee.incentive_category || row.category // Override category if exists
                };
            }
            return row;
        });
